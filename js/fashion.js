...
const gradients = {
  ...
  grad5: 'linear-gradient(135deg,#667eea 0%,#764ba2 100%)',
  grad6: 'linear-gradient(135deg,#a8edea 0%,#fed6e3 100%)'
};
...
function getEmojiForType(t) {
  return { sneakers: '👟', bags: '👜', jackets: '🧥', accessories: '🕶️' }[t] || '👗';
}

function isFashionAuction(item) {
  if (!item) return false;
  const t = (item.type || '').toLowerCase();
  const s = (item.subType || '').toLowerCase();
  return t === 'fashion' || s === 'fashion';
}

function openDetails(item) {
  if (!item) return;

  localStorage.setItem(selectedAuctionSourceKey, 'fashionAuctions');
...
function buildFashionCard(x, badgeMode = 'normal') {
...
function bindFashionCards(container, items) {
  if (!container) return;
...
function renderCategoryShowcase() {
  if (!categoryShowcaseGrid || !categoryShowcaseSection) return;

  const items = loadItems().filter(isFashionAuction);
  ...
  categoryShowcaseGrid.innerHTML = showcaseItems.map(x => buildFashionCard(x, 'showcase')).join('');
bindFashionCards(categoryShowcaseGrid, showcaseItems);
} 
function render() {
  const items = loadItems().filter(isFashionAuction);
  ...
  if (!filtered.length) {
    grid.innerHTML = '<div class="empty">No fashion items found.</div>';
    return;
  }

  grid.innerHTML = filtered
    .map(x => buildFashionCard(x, 'normal'))
    .join('');

  bindFashionCards(grid, filtered);
}
