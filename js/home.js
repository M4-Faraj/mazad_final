...
const PAGE_CATEGORY = 'home';
const gradients = {
  grad1: 'linear-gradient(135deg,#43e97b 0%,#38f9d7 100%)',
...
function getEmojiForType(t) {
  return { furniture: '🛋️', decor: '🪞', kitchen: '🍳', garden: '🌿' }[t] || '🏡';
}

function isHomeAuction(item) {
  if (!item) return false;
  const t = (item.type || '').toLowerCase();
  const s = (item.subType || '').toLowerCase();
  return t === 'home' || s === 'home';
}

function openDetails(item) {
  if (!item) return;

  localStorage.setItem(selectedAuctionSourceKey, 'homeAuctions');
...
function render() {
  const items = loadItems().filter(isHomeAuction);
  ...
  if (!filtered.length) {
    grid.innerHTML = '<div class="empty">No home items found.</div>';
    return;
  }
