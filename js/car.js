...
const PAGE_CATEGORY = 'cars';
const gradients = {
  grad1: 'linear-gradient(135deg,#4facfe 0%,#00f2fe 100%)',
  grad2: 'linear-gradient(135deg,#30cfd0 0%,#330867 100%)',
  grad3: 'linear-gradient(135deg,#667eea 0%,#764ba2 100%)',
...
function getEmojiForType(t) {
  return { sedan: '🚗', suv: '🚙', sports: '🏎️', classic: '🚘', truck: '🛻' }[t] || '🚗';
}

function isCarAuction(item) {
  if (!item) return false;
  const t = (item.type || '').toLowerCase();
  const s = (item.subType || '').toLowerCase();
  return t === 'cars' || s === 'cars';
}

function openDetails(item) {
  if (!item) return;

  localStorage.setItem(selectedAuctionSourceKey, 'carsAuctions');
...
function buildCategoryCard(x, badgeMode = 'normal') {
  const imageUrl = (x.images && x.images.length) ? x.images[0] : (x.imageUrl || x.image || '');
  const gradient = gradients[x.img] || gradients.grad2;
      ...
      <div class="card-overlay">
        ...
        <div class="overlay-bottom">
          <div>
            ...
            <div class="meta">
              by ${escapeHtml(x.seller || 'Unknown')}${x.year ? ' · ' + x.year : ''}
              ${FavoritesService.buildRankBadge(x.sellerRank, x.sellerTokens)}
            </div>
          </div>
...
function render() {
  const items = loadItems().filter(isCarAuction);
  ...
  let filtered = Array.from(seen.values()).filter(x =>
    ...
    String(x.year || '').includes(query)
  );

  if (s === 'new') filtered.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
  if (s === 'low') filtered.sort((a, b) => (a.price || 0) - (b.price || 0));
  if (s === 'high') filtered.sort((a, b) => (b.price || 0) - (a.price || 0));
  if (s === 'yearNew') filtered.sort((a, b) => (b.year || 0) - (a.year || 0));
  if (s === 'yearOld') filtered.sort((a, b) => (a.year || 0) - (b.year || 0));
