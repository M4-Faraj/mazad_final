...
function getEmojiForType(t) {
  return { watch: '⌚', ring: '💍', earrings: '💎', necklace: '✨' }[t] || '💎';
}

function isWatchAuction(item) {
  if (!item) return false;
  const t = (item.type || '').toLowerCase();
  const s = (item.subType || '').toLowerCase();
  return t === 'watch' || t === 'watches' || s === 'watch' || s === 'watches';
}

function openDetails(item) {
  if (!item) return;

  localStorage.setItem(selectedAuctionSourceKey, 'watchesAuctions');
...
function render() {
  const items = loadItems().filter(isWatchAuction);
  ...
  if (!filtered.length) {
    grid.innerHTML = '<div class="empty">No watches or jewelry found.</div>';
    return;
  }
