...
const gradients = {
  ...
  grad4: 'linear-gradient(135deg,#f093fb 0%,#f5576c 100%)',
...
function getEmojiForType(t) {
  return { painting: '🖼️', vintage: '🏺', coins: '🪙', cards: '🃏' }[t] || '🎨';
}

function isArtAuction(item) {
...
function openDetails(item) {
  ...
  localStorage.setItem(selectedAuctionSourceKey, 'artAuctions');
...
function render() {
  const items = loadItems().filter(isArtAuction);
  ...
  if (!filtered.length) {
    grid.innerHTML = '<div class="empty">No art items found.</div>';
    return;
  }
